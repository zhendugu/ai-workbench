...
function hasUnknownFields(obj, allowedFields) {
    const unknown = [];
...
function validateIdentifierFormat(value, prefix, path) {
    const errors = [];
...
export function validateCompany(value, path = 'company') {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
...
export function validateCell(value, path = 'cell') {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
...
const VALID_CONSTRAINT_TYPES = ['allow', 'forbid', 'condition'];
export function validateRole(value, path = 'role') {
    ...
    else if (!VALID_CONSTRAINT_TYPES.includes(value.constraint_type)) {
        errors.push({
    ...
    if (!isString(value.attached_to_cell_identifier)) {
        ...
        errors.push(...validateIdentifierFormat(value.attached_to_cell_identifier, 'CELL', `${path}.attached_to_cell_identifier`));
    }
    return {
        valid: errors.length === 0,
        errors,
...
const VALID_RELATIONSHIP_TYPES = ['dependency', 'reference', 'input_output'];
export function validateRelationship(value, path = 'relationship') {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
    ...
    else if (!VALID_RELATIONSHIP_TYPES.includes(value.relationship_type)) {
        errors.push({
...
export function validateTopology(value, path = 'topology') {
    const errors = [];
    if (!isArray(value)) {
        return {
            valid: false,
...
export function validateMethodology(value, path = 'methodology') {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
    ...
    if (!isString(value.methodology_identifier)) {
        ...
        errors.push(...validateIdentifierFormat(value.methodology_identifier, 'METH', `${path}.methodology_identifier`));
    }
...
export function validateFreezeRecord(value, path = 'freeze_record') {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
    ...
    if (!isString(value.freeze_record_identifier)) {
        ...
        errors.push(...validateIdentifierFormat(value.freeze_record_identifier, 'FREEZE', `${path}.freeze_record_identifier`));
    }
    if (!isString(value.frozen_company_identifier)) {
        ...
        errors.push(...validateIdentifierFormat(value.frozen_company_identifier, 'FROZEN-COMP', `${path}.frozen_company_identifier`));
    }
...
// ============================================================================
export function validateFrozenSnapshot(value) {
    const errors = [];
    if (!isObject(value)) {
        return {
            valid: false,
    ...
    if (isArray(value.cells) && isArray(value.roles)) {
        const cellIds = new Set(value.cells.map((c) => c.cell_identifier));
        value.roles.forEach((role, index) => {
    ...
    if (isArray(value.topology) && isArray(value.cells)) {
        const cellIds = new Set(value.cells.map((c) => c.cell_identifier));
        value.topology.forEach((rel, index) => {
